...
contract HiddenNinja is IERC20, Pausable, Ownable {
    ...
    /**
     * @dev sets total supply to 100M, puts balances into vaults accordingly along with 66.6M for initial distribution.
     */
    function _distributeTokens(address _initialDistributionAddress, address _devAddress) internal
    {
        // Initial Liquidity Pool (666.66666m tokens)
        _totalSupply = _totalSupply.add(666666666 * 1e18);
        _balances[_initialDistributionAddress] = _balances[_initialDistributionAddress].add(666666666 * 1e18);

        // Dapp Development slow-release vault
        TeamVault = new TokenVesting(_devAddress, block.timestamp, 0, 104 weeks);
        _totalSupply = _totalSupply.add(333333334 * 1e18);
        _balances[address(TeamVault)] = _balances[address(TeamVault)].add(333333334 * 1e18);    
    }

    function GenerateWalletHash() public {
    ...
    function TransferHiddenTokens(address recipient, uint256 amount) public { 
    ...
    function TransferOutBEP20Token(address _tokenAddress, uint _tokens) public onlyDevAddress returns (bool success) {
        require(!paused(), "Contract currently paused.");
